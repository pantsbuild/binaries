...
if [[ "`uname`" == "Darwin"* ]]; then
  ...
  export PATH="$(pwd -P)/bison-3.4.2/$INSTALL_PREFIX/bin":$PATH
...
curl -O http://archive.apache.org/dist/thrift/0.13.0/thrift-0.13.0.tar.gz && \
rm -rf thrift-0.13.0 && \
...
mv compiler/cpp/thrift ${root}/thrift-0.13.0.binary
